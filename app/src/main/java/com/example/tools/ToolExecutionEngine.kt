...
class ToolExecutionEngine(private val context: Context) {
    ...
    fun getBatteryStatus(): String {
        return try {
            ...
            val chargeText = if (isCharging) "charging hai" else "charging nahi ho rahi"
            "Battery level is $batteryPct%, device $chargeText."
        } catch (e: Exception) {
    ...
    fun getCurrentTimeAndDate(): String {
        val sdf = java.text.SimpleDateFormat("hh:mm a, EEEE, dd MMMM yyyy", java.util.Locale.getDefault())
        return "Current time and date: " + sdf.format(java.util.Date())
    }
    ...
    fun callContact(nameOrNumber: String, useDialer: Boolean = false, simSlot: Int? = null): String {
    ...
    fun playMedia(query: String): String {
        try {
